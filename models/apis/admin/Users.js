...
const signUpSchema = Joi.object({
	...
	firstName1: Joi.string()
		...
		.message(
			"First letter must be uppercase and contain both uppercase and lowercase letters"
		)
		.when("$isEdit", {
			is: false,
			then: Joi.required(),
	...
	password1: Joi.string()
		...
		.message(
			"password includes at least 1 uppercae , 1 lower case letter and 1 specil character"
		)
		.when("$isEdit", {
			is: false,
			then: Joi.required(),
	...
	country1: Joi.string().when("$isEdit", {
		is: false,
		then: Joi.required(),
		otherwise: Joi.optional(),
	}),
	...
	image1: Joi.string()
		.regex(validExtensions)
		.message("allowed image ext. are .jpg/.jpeg/.png/.svg/.gif")
		.when("$isEdit", {
			is: false,
			then: Joi.required(),
			otherwise: Joi.optional(),
	...
	oldPassword1: Joi.string().when("$ispasswordChange", {
		is: true,
		then: Joi.required(),
		otherwise: Joi.optional(),
	}),
